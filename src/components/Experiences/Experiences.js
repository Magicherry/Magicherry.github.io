      ...
      <Container>
        ...
        <VerticalTimeline>
          {internshipExperience.map((experience, index) => {
            const titleParts = experience.title.split(",");
            const jobTitle = titleParts[0];
            const department = titleParts.length > 1 ? titleParts.slice(1).join(",").trim() : "";
            ...
            return (
              ...
              >
                ...
                <h4 className="vertical-timeline-element-subtitle">{department}</h4>
